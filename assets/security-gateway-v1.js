/* RSJ Security Gateway V2 — same-origin RPC proxy + IP/device audit */
...
window.fetch=async function(input,init){
  ...
  if(rpc==='self_register_member'||rpc==='member_session_lucky_draw_safe'){
    ...
    return originalFetch('/api/security-gateway',{...common,body:JSON.stringify(payload)});
  }
  return originalFetch('/api/rpc?name='+encodeURIComponent(rpc),{...common,body:JSON.stringify(args)});
};
})();
